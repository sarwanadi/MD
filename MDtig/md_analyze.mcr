...
# Delphi map and then visualized automatically by the md_play macro. The grid resolution is set with the SolvPar command.
...
def PrintLigAtomIntInfo
  ...
  WriteReport Image,Filename=(resultbase)_ligandatomnum.png,Style=Figure,
    Caption='A ray-traced picture of the ligand with atoms labeled by their respective atom number',Delete=Yes
...
# Remember cell size of the water scene for e.g. ESP
...
while !last and snapshots!=0
  ...
  if espmethod!=''
    ...
    # Set cell size to the one of the water scene
    Cell (cellsize)
    ...
    tbl=LoadTab (resultbase)(i).phi.gz
    vallist()=Tab (tbl)
    if i==firstsnapshot
      esptbl=tbl
      espvallist=vallist
...
if espmethod!=''
  # Clculate and save average ESP
  espvallist()=espvallist()/snapshots
  SelectTab (esptbl),Column=1
  Tabulate (espvallist)
  SaveTab (esptbl),(resultbase)_average.phi.gz,Format=Delphi,NumFormat=11.5f
  # Visualize average ESP
  AddObj SoluteRef
  SwitchObj not SimCell SoluteRef,off
  Cell (cellsize)
  esp=LoadESP (resultbase)_average.phi.gz
  ...
  WriteReport Paragraph,
    ...
    'to "file://(basename resultbase)(lastsnapshot).phi.gz" and the average in "file://(basename resultbase)_average.phi.gz".'
  if (espmethod=='PBS')
    WriteReport Paragraph,
...
def PlotMap type,name,sellist(),valuelist(),valmin,valmax,title,graphnamestr,graphcolstr
  ...
  for i=1 to count mollist
    selections=Count(type) (mollist(i)) (join sellist)
    if selections
      if task=='AddHeader'
        # Create tables
